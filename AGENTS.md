...
- 叶子 `sensor::*` 参数类型保持 POD + STL（`std::array`）；Eigen 已是 `phad_sensor`、`phad_common`、`phad_eval` 的 public 依赖（`RigidTransform`、`common::Trajectory` 用 `Eigen::Isometry3d`）；spdlog 1.17.0 备在 gitignore 的 `thirdparty/` 尚未接入；GTSAM 4.3.0 已装在 `/usr/local`，经 `find_package(GTSAM 4.3 REQUIRED)` 由 `phad_estimator` PRIVATE 链接并把 include dir 标 SYSTEM 以挡警告刷屏，且其 Eigen 必须与 `Eigen3::Eigen` 3.4 是同一份（构建 GTSAM 时用 `GTSAM_USE_SYSTEM_EIGEN=ON`）
