...
- 里程碑顺序与出口以 `docs/roadmap.md` 为准：M1→M2.x→M3.1→M3.2 已完成；M3.3 Slice ①②③④d 已完成；④/④b MH_05 不够；④c（`block_culled_rebirth`+`dropTracks`）部分完成后由 ④d 收口；④e（`0ced28b`/`default_3a21162e`，`max_outlier_reopts=3`）MH_05 持平（≈4.565）→ 部分完成；④f（`c446ac5`/`default_a5e90dc7`，`skip_drop_min_culled=4`）MH_01 硬门 PASS、MH_05 软门 PASS（ATE ≈3.057 vs ④e ≈4.565）→ **已完成**、全序列未跑；MH_05 Probe B（`--probe-b` 旁路 jsonl，默认关、不进 `config_hash`、不改 18 列 `diag.csv`）**已完成**（merge `8a94fd3`）：证实 i=425 skip-drop 后 zombie×`block_culled_rebirth` 导致 i=429 低残差大位移锁定（ATE 仍 ≈3.057，与 ④f bit-identical）；④g（`3ee5dea`/同 hash `a5e90dc7`，skip 后延后精确 `dropTracks`）MH_01 硬门 PASS、MH_05 软门 **FAIL**（ATE **4.565065**=④e 平台；`drops_skipped=1`；`deferred_drops=1`；`deferred_drop_ids=32`；zombie 归零但 427–429 shared 29/43/52 未收窄）→ **门控不够 / 部分完成**、停全序列；解读：延后 drop 清 zombie 但复现 ④e drop 伤害，④f skip-without-any-drop 在 ATE 上仍更好；下一刀=**TBD**（按 ④g 失败模式另议；否决单独去 Huber/观测级/关整个 block/关整个 skip-drop/⑤ 作首选）；`LandmarkId` 现为 frontend track 与 estimator map 共用身份，TrackId≠LandmarkId 为已确认的中期债；⑤（关键帧；与 M4 预积分耦合）仍先对齐再开；前端 OpenCV、后端 GTSAM，自研以调库版为对拍 oracle
