...
- 里程碑顺序与出口以 `docs/roadmap.md` 为准：M1→M2.x→M3.1→M3.2 已完成；M3.3 Slice ①②③④d 已完成；④/④b MH_05 不够；④c（`block_culled_rebirth`+`dropTracks`）部分完成后由 ④d 收口（`79505f8`/`default_85c97158`，默认 `outlier_avg_reproj_px=4.0`，MH_01 ATE ≈0.0988 m，MH_05 仍 ≈4.565）；`LandmarkId` 现为 frontend track 与 estimator map 共用身份，TrackId≠LandmarkId 为已确认的中期债；下一建议 ④e 多轮 cull↔LM（⑤ 门控已满足但与 M4 耦合）；前端 OpenCV、后端 GTSAM，自研以调库版为对拍 oracle
