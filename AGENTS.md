...
- 里程碑为 M1 数据 IO（已完成）→ M2 双目 VO 最小闭环（M2.1–M2.3 已完成，issue `#20`，计划 `docs/plans/2026-07-31_m2.3_vo_backend_dcdbfc71.plan.md`；MH_01 基线 ATE translation RMSE ≈ 0.15 m，拒帧/连通性/RMS 只记录不门控）→ M3.1 回归 Benchmark（已完成，issue `#21`，设计 `docs/research/m3.1-vo-regression-benchmark-design.md`，计划 `docs/plans/2026-07-31_m3.1_vo_regression_benchmark_7c4e91a2.plan.md`；`phad::bench` + `OfflineVoSession` + `phad_vo_bench` + `scripts/bench_table.py`；MH_01 ATE translation RMSE ≈ 0.150155 m）→ M3.2 双目配对同步器（已完成，issue `#22`，设计 `docs/research/stereo-pair-synchronizer-design.md`，计划 `docs/plans/2026-07-31_m3.2_stereo_pair_synchronizer_5b7d1c93.plan.md`；`phad::sync` + per-camera dataset API + `StereoPairStream`；MH_01 相对 M3.1 基线逐字节相同；11/11 EuRoC `open`+`summary.json`；MH_04/V1_02/V2_03 emit/drop 精确匹配）→ M3.3 VO 加固 → M4 接入 IMU → M5 正式初始化 → M6 边缘化 → M7 线程 → M8 解耦回环 → M9 研究平台；每个里程碑以真实序列上的 ATE 或可观察行为收尾；前端用 OpenCV、后端用 GTSAM，自研实现以调库版本为对拍 oracle，不通过对拍则两者并存
