...
- `phad::viz` 拆成可无显示测试的 `TrajectoryPanel`（俯视 x-y，渲染到 `cv::Mat`）与薄的 `ImageWindow`（highgui 窗口与退出键）；`tests/viz` 只渲染内存画布，不开窗口。GUI 端到端（`phad_euroc_runner` 回放）需要人工确认，agent 不擅自弹窗；`phad_viz` 把 `opencv_core` 设为 PUBLIC，新库若照做会让 OpenCV 传染到下游 estimator
...
- 单元测试在 `build/` 下通过 `ctest --output-on-failure -L unit` 运行，也可直接运行各 suite 二进制（`phad_sensor_tests`、`phad_camera_tests`、`phad_io_dataset_tests`、`phad_eval_tests`、`phad_viz_tests`）；本地 EuRoC 序列在 `/home/lin/Projects/data/thidparty/euroc/native/<sequence>`，`PHAD_ENABLE_MH01_TESTS=ON` 加 `PHAD_EUROC_MH01_PATH` 启用 `-L mh01` 测试（`phad_euroc_mh01_test`、`phad_eval_mh01_test`）
...
- 里程碑为 M1 数据 IO（已完成）→ M2 双目 VO 最小闭环（M2.1 评估与可视化底座已完成） → M3 VO 加固 → M4 接入 IMU → M5 正式初始化 → M6 边缘化 → M7 线程 → M8 解耦回环 → M9 研究平台；每个里程碑以真实序列上的 ATE 或可观察行为收尾
