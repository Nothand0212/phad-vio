...
可据此区分「算法变好」与「re-anchor / PnP fallback / 剔点空转变多」。
`est.tum` 仍为单条连续轨迹，不含段号列。
...
`estimator.enable_outlier_cull` / `estimator.outlier_avg_reproj_px`（apps
侧 `flattenConfig` 展平）；新配置落新目录。
