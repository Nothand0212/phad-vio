...
| Glue 落点 | `apps/stereo_vo_glue.hpp`（薄 helper，probe / runner 共用）；不进任何 `phad_*` 库 |
...
| 首帧与共视 | 首帧豁免 `num_shared == 0`（`T_W_B = I` + backproject → `kOk`）；共视拒帧只在已初始化之后生效 |
| `kFailed` 边界 | 仅优化抛异常（如 `IndeterminantLinearSystemException`）或结果含非有限值；大残差 / `low_connectivity` / cheirality 不升格为 `kFailed` |
| MH_01 出口 | probe 不崩 + TUM 写出 + 有限 ATE 且 RMSE **&lt; 5 m**；拒帧比例 / `low_connectivity` / 重投影 RMS **只记录、不门控** |
...
flowchart LR
  ...
  Tracker -->|"FrameTracks"| Glue["apps/stereo_vo_glue.hpp: filter kValid"]
...
min_shared_landmarks` 仍然优化但置 `low_connectivity`；**已初始化之后**
`num_shared == 0` 直接 `kRejected`——此时新帧位姿完全自由，因为它自己的新
landmark 恰好被自己的观测唯一确定，LM 会把它留在初值上并报告一个很小的 error，
即一次静默的假成功。**首帧豁免**该规则（landmark 表为空时 `num_shared` 必然为 0）。

`kFailed` 仅在优化抛异常或结果含非有限值时；大残差、`low_connectivity`、
cheirality 不升格。
...
- 用外参 A 生成观测、构造时喂外参 B，断言位姿误差显著且状态仍为 `kOk`。错误外参
  **不会**返回 `kFailed`；BA 可通过右乘把错误 `body_P_sensor` 吸进 `T_W_B`（残差可
  接近 0），Umeyama 对齐消不掉，故判据是位姿误差而非状态码/残差。

## 步骤 10：probe 与 runner（Slice ④）

`FrameTracks` → `KeyframeMeasurement` 的转换落在 `apps/stereo_vo_glue.hpp`，由
probe 与 runner 共用，不进 `phad_*` 库。
...
**通过条件**：probe 跑完不崩、TUM 写出、ATE 有限且 RMSE **&lt; 5 m**。拒帧比例、
`low_connectivity` 帧数与重投影 RMS 分布**只记录、不门控**——否则「ATE 很好」可能
只是因为丢掉了难的那些帧，M3 也就无从对比。roadmap 出口里的「完整轨迹文件」定义为：
轨迹覆盖序列的完整时间跨度，写出的位姿数等于 `kOk` 帧数，且被拒帧数与原因分布被显式
报告。
