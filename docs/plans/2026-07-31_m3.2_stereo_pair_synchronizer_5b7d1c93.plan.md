...
todos:
  ...
  - id: reference-artifacts
    content: 改动前在当前 commit 用 phad_vo_bench 跑 MH_01，把 est.tum 与 diag.csv 存到仓库外作逐字节对拍参考
    status: pending
...
`summary.json` 只新增同步诊断字段，既有字段名与含义不变，`scripts/bench_table.py`
无需改动。
...
diff <新 est.tum> <参考 est.tum>      # 必须为空
diff <新 diag.csv> <参考 diag.csv>    # 必须为空
```
