...
日期：2026-08-02  
状态：**已编码** — MH_05 **不够**（持平 ④d ATE **4.565**；无软改善）→ 停全序列；
MH_01 不回归（ATE **0.0987839**）。默认 `max_outlier_reopts=3`
（`0ced28b` / `default_3a21162e`）。对照锚 ④d：`79505f8` / `default_85c97158`。  
相关：
...
- 基线 [`m3.3-slice4-baseline.md`](m3.3-slice4-baseline.md) §10（④e）/ §9（④d）
- 计划 [`2026-08-02_m3.3_slice4e_multiround_reopt_e0517b9a.plan.md`](../plans/2026-08-02_m3.3_slice4e_multiround_reopt_e0517b9a.plan.md)
...
| MH_05 | 3.45（不够） | 4.565 | **4.565 持平（不够）**；多轮未动用 |
| A/B | `enable_reopt=false` | CLI T | `max=1` / `enable=false` |
| 下一刀 | — | 建议 ④e | 不够 → 去 Huber / 观测级续片；⑤ 先对齐再开 |

Slice ⑤（关键帧）仍单独对齐；④e 不够 → 本阶段不开 ⑤。
