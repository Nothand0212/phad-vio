...
日期：2026-08-01  
状态：已对齐（含评审修订：LM₂ 失败撤销 LM₂ 写回 kOk、触发 ≥4、`culled_ids_` 快照）；已编码；MH_05 诊断门不够（ATE 3.45 m）  
相关：
...
| 失败 | LM₂ 失败 → 撤销 LM₂ 写回，保留「LM₁ 写回 + cull 后」状态 → `kOk`、`outlier_reopt=false`、`outlier_reopt_failed=true` |
...
struct UpdateDiagnostics
{
  ...
  bool outlier_reopt_failed = false;  // LM₂ 失败已撤销 LM₂ 写回（保留 LM₁ 写回 + cull）；不进 diag.csv
};
```
...
→ if enable_outlier_reopt && outliers_culled >= 4:
     ...
     // LM₂ 失败：撤销 LM₂ 写回，保留「LM₁ 写回 + cull 后」状态
...
**运行时**：不定系统 / 非有限等仍走既有 `kFailed`。**LM₂ 失败单独处理**：撤销 LM₂ 写回，保留「LM₁ 写回 + cull 后」状态走 `kOk`（`outlier_reopt=false`、`outlier_reopt_failed=true`）。理由：剔点越多越可能欠约束（LM₂ 抛 `IndeterminantLinearSystemException`），而这些帧在 Slice ④ 本是 `kOk`，LM₂ 失败不应把好帧变 `kFailed`（completion 与 eval 空洞恶化，与改善 MH_05 的目标相悖）；LM₁ 写回是 ④ 已验证状态，撤销 LM₂ 写回严格不差于 ④。
...
| LM₂ 失败撤销写回 | 构造剔点后欠约束图（LM₂ 抛异常）→ `kOk`、`outlier_reopt==false`、`outlier_reopt_failed==true`、位姿为 LM₁ 写回、cull 保留 |
